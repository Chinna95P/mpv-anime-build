...
-- debounced OSD: last message wins after 200ms delay
local osd_timer = nil
local function osd(msg, duration)
    duration = duration or 3
    if osd_timer then osd_timer:kill() end
    osd_timer = mp.add_timeout(0.2, function()
...
local options = {
    -- behavior
    mode = 2, -- [0-4] more details above; 2=one-shot (default), 4=continuous
    start_delay = 3, -- delay in seconds used to skip intro (usefull with mode 2)
...
local function switch_hwdec(id, hwdec, error)
    if hwdec ~= "no" and not string.match(hwdec, "-copy") then
        ...
        mp.msg.info(msg)
        osd(string.format("%s: %s", label_prefix, msg), 5)
    end
...
local function collect_metadata(event)
    if event.prefix == "ffmpeg" and event.level == "v" and string.find(event.text, "^.*dyn_cd: ") and
        ...
        if s.in_progress or not s.timestamps.previous or wait_limit or s.f_inserted or (mp.get_time() - s.real_start_time) < options.start_delay then
            s.f_inserted = false
            return
        end
...
function on_toggle(auto)
    if s.f_missing then
        osd("Libavfilter cropdetect missing", 3)
        return
    end
    local EVENT = "toggle"
    if s.toggled == ENABLE then
        ...
        seek(EVENT)
        if not auto then osd(string.format("%s: disabled, crop remains.", label_prefix), 3) end
    elseif s.toggled == DISABLE_WITH_CROP then
        ...
        if not auto then osd(string.format("%s: crop removed.", label_prefix), 3) end
        ...
        resume(EVENT)
        if not auto then osd(string.format("%s: enabled.", label_prefix), 3) end
    end
end
...
local function on_start()
    ...
    if not is_cropable() then
        -- track-list may not be populated yet on this playback path (seen on
        -- Android/mediacodec); retry a few times before giving up.
        local retries = 10
        local function retry_check()
            ...
            mp.add_timeout(0.1, retry_check)
        end
        mp.add_timeout(0.1, retry_check)
        return
    end
...
-- cycle between one-shot (mode 2) and continuous (mode 4)
-- works on current playback, not just next file
-- tap:        mp.command("script-message cycle-crop-mode")
-- long press: mp.command("script-message reset-crop-mode")
mp.register_script_message("cycle-crop-mode", function()
    if not s.started then
        osd("Crop: no video loaded", 2)
        return
    end

    local speed = tonumber(mp.get_property("speed"))
    if speed and speed >= 1.2 then
        osd("Crop: locked (fast playback)", 2)
        return
    end
    ...
    if options.mode == 4 then
        ...
        osd("Crop: Continuous", 2)
    end
end)

mp.register_script_message("reset-crop-mode", function()
    if not s.started then return end
    options.mode = 2
    s.toggled = ENABLE
    s.seek_done = false
    osd("Crop: Reset to One-shot", 2)
end)